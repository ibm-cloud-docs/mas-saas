...
Note: typically a site-to-site VPN is required to make a connection from the MAS-SaaS environment to your LDAP server.
...
2. Case description: Please configure our MAS-SaaS environment to use the following SMTP server:
...
* Note: It is important to bear in mind a VPN may not necessarily be needed to establish certain types of connectivity. Some integration types can run over HTTPS and/or Cloud Object Storage (AWS S3) and may not require a VPN.
...
A Self Service Portal (SSP) for Maximo Application Suite as a Service (MAS-SaaS) is not available at this time and is under construction.